 ...
 * 用于启动消息处理中心
 */
public class BrkServer implements Runnable {

    private static final Logger logger = LoggerFactory.getLogger(BrkServer.class);

    private final Socket socket;

    public BrkServer(Socket socket) {
        this.socket = socket;
    }
        ...
        try (BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
            ...
            while (true) {
                ...
                if (str.startsWith(Config.PUBLISH)) {
                    ...
                    String stageName = strs[1];
                    String message = strs[2];
                    Broker.handlePublish(stageName,message);
                    logger.info("成功publish，此时的信息列表为："+Broker.receivedMessagesMap);

                } else if (str.startsWith(Config.SUBSCRIBE)) {
                    //接受到的请求包含SEND:字符串 表示生产消息放到消息队列中
                    String [] strs=str.split("\t",3);
                    String userName = strs[1];
                    String stageName = strs[2];
                    Broker.handleSubscribe(userName,stageName);
